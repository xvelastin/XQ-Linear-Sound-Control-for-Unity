...
[CustomPropertyDrawer(typeof(XQ))]
public class XQDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        XQGUIUtility.ColorAllTextInStyle(PlayButtonStyle, XQGUIUtility.QTypeToColor(CueType.Play));

        GUIStyle FadeButtonStyle = new GUIStyle(EditorStyles.miniButton);
        FadeButtonStyle.fixedHeight = buttonLines * singleLineHeight;
        XQGUIUtility.ColorAllTextInStyle(FadeButtonStyle, XQGUIUtility.QTypeToColor(CueType.Fade));

        GUIStyle StopButtonStyle = new GUIStyle(EditorStyles.miniButton);
        StopButtonStyle.fixedHeight = buttonLines * singleLineHeight;
        XQGUIUtility.ColorAllTextInStyle(StopButtonStyle, XQGUIUtility.QTypeToColor(CueType.Stop));
        ...
        if (property.isExpanded)
        {
            ...
            if (AddPlayButton)
            {
                List<XQ> thisQList = fieldInfo.GetValue(property.serializedObject.targetObject) as List<XQ>;
                XQ thisStackedQ = thisQList[stackedQIndex];
                ...
                drawHeaderCallback = position =>
                    {
                        ...
                        position = XQGUIUtility.MoveRect.Shift(position, textWidth, 0, singleLineHeight);
                ...
                drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                    {
                        ...
                        // Update qIndex property for each Q
                        SerializedProperty selectedQ = qs.GetArrayElementAtIndex(index);
                        selectedQ.FindPropertyRelative("qIndex").intValue = index;
                ...
                elementHeightCallback = index =>
                    {
                        ...
                        if (selectedQ.isExpanded)
                        {
                            ...
                            int qtypelines = XQGUIUtility.QTypeToLines(cueTypeAsEnum);
        ...
        if (property.isExpanded)
        {
            ...
            propertyHeight = rlist.GetHeight() + singleLineHeight;
        }
        else
        {
